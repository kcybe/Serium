...
import { Tags, ListTodo, Settings2, ListFilter, Trash2 } from "lucide-react";
...
export function TableSettings({
  ...
  const handleAddCustomColumn = () => {
    form.setValue("customColumns", [
      ...form.getValues("customColumns"),
      {
        id: uuidv4(),
        label: "",
        type: "text",
  ...
  const handleRemoveCustomColumn = (index: number) => {
    ...
    form.setValue(
      "customColumns",
      currentCustomColumns.filter((_, i) => i !== index)
    );
  };
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
        ...
        >
          <div className="grid gap-4">
            {Object.entries(form.getValues().visibleColumns).map(
              ([column, isVisible]) => (
                <div key={column} className="flex items-center justify-between">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) => {
                      ...
                      form.setValue(
                        `visibleColumns.${
                          column as keyof typeof settings.visibleColumns
                        }`,
                        checked
                      );
                    }}
        ...
        >
          <div className="flex flex-col gap-4">
            {form.watch("customColumns").map((column, index) => (
              <div
                key={column.id}
                className="grid grid-cols-[1fr_1fr_auto_auto] items-start gap-4"
              >
                <FormField
                  control={form.control}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{t("table.columns.type")}</FormLabel>
                      <Select
                        onValueChange={field.onChange}
                        defaultValue={field.value}
                      >
                        <FormControl>
                          <SelectTrigger>
                            <SelectValue
                              placeholder={t("general.form.typePlaceholder")}
                            />
                          </SelectTrigger>
                        </FormControl>
                        <SelectContent>
                          <SelectItem value="text">
                            {t("table.columnTypes.text")}
                          </SelectItem>
                          <SelectItem value="number">
                            {t("table.columnTypes.number")}
                          </SelectItem>
                          <SelectItem value="boolean">
                            {t("table.columnTypes.boolean")}
                          </SelectItem>
                        </SelectContent>
                      </Select>
                      <FormMessage />
                ...
                <div className="flex items-center space-x-2 mt-8">
                  <Switch
                    ...
                    checked={form.watch(`customColumns.${index}.required`)}
                    onCheckedChange={(checked) => {
                      form.setValue(`customColumns.${index}.required`, checked);
                    }}
                  />
                  <Label htmlFor={`${column.id}-required`}>
